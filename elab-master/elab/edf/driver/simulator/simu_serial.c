...
#define SIMU_SERIAL_SLAVE_MAX               (32)
...
typedef struct simu_serial
{
    ...
    struct simu_serial *slave[SIMU_SERIAL_SLAVE_MAX];
    uint8_t slave_count;
} simu_serial_t;
                                        ...
                                        uint8_t mode, uint32_t baudrate)
{
    ...
    memset(serial, 0, sizeof(simu_serial_t));
    ...
    if (mode == SIMU_SERIAL_MODE_485_M)
    {
        attr.mode = ELAB_SERIAL_MODE_HALF_DUPLEX;
    }
    elab_serial_register(&serial->device, name, &_serial_ops, &attr, serial);

    /* Register the simulated serial device into the hash table. */
    elab_err_t ret = hash_table_add(ht_simu, (char *)name, serial);
...
{
    elab_device_t *dev_m = elab_device_find(name);
    simu_serial_t *serial_m = (simu_serial_t *)dev_m->user_data;

    elab_assert(serial_m->mode == SIMU_SERIAL_MODE_485_M);

    elab_device_t *dev_s = elab_device_find(name_slave);
    simu_serial_t *serial_s = (simu_serial_t *)dev_s->user_data;

    /* Check the slave is in the master's slave table. */
    for (uint32_t i = 0; i < serial_m->slave_count; i ++)
    {
        elab_assert(serial_m->slave[i] != serial_s);
    }
    serial_m->slave[serial_m->slave_count ++] = serial_s;
    elab_assert(serial_m->slave_count < SIMU_SERIAL_SLAVE_MAX);

    serial_s->partner = serial_m;
}
...
static int32_t _write(elab_serial_t *serial, const void *pbuf, uint32_t size)
{
    ...
    if (simu_serial->mode == SIMU_SERIAL_MODE_SINGLE)
    {
        ...
        for (uint32_t i = 0; i < size; i ++)
        {
            ...
            elab_assert(ret == osOK);
             ...
             simu_serial->mode == SIMU_SERIAL_MODE_485_S)
    {
    ...
    {
        simu_serial_t *slave = NULL;
        for (uint32_t s = 0; s < simu_serial->slave_count; s ++)
        {
            slave = simu_serial->slave[s];
